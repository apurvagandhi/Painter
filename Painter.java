...
public class Painter extends AsyncEventAdapter
{
	...
	public static void respondToRightClick(int x, int y)
    {
		...
		// But only do this if the pixel is not already painted that color.
       	if (!canvas.colorAt(x, y).equals(canvas.getPenColor()))
        {
            //calls the bucket paint
            bucketPaint(x, y, canvas.colorAt(x,y));
		}
    }//end of respondToRightClick
    
    //Call to this function will recursively call itself and
    //fill in the pixel accordingly
    public static void bucketPaint (int x, int y, Color color)
    {
        ...
        //error checks for the color at (x,y)
        if (!(color.equals(canvas.colorAt(x, y))))
        {
            return;
        }
        //Doesb the  acutal paint 
      	canvas.point(x, y);
        //Recursively calls the bucket paint with incremented parameters 
        bucketPaint(x,y+1, color);
        bucketPaint(x,y-1, color);
        bucketPaint(x+1,y, color);
        bucketPaint(x-1,y, color);
    }//end of bucketPaint
}//end of Main
